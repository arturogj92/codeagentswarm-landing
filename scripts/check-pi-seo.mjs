...
for (const pair of pairs) {
  ...
  for (const [i, guide] of guides.entries()) {
    ...
    assert.ok(images.some(b => b.src === '/icons/apps/pi-icon.svg'), 'Pi icon missing')
    assert.ok(images.some(b => b.src.startsWith('/images/guides/pi-') && b.caption && b.alt), 'captioned Pi capture missing')
